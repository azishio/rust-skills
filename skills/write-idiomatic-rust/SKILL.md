...
- `clippy::nursery` runs at warning level only. Treat its findings as review candidates: assess them against the target project's requirements, MSRV, public-API compatibility, measurements, and local conventions. Do not change code, add an abstraction, or add a lint exception solely to silence a Nursery warning.
- `single_call_fn`, `multiple_inherent_impl`, `exhaustive_enums`, and `exhaustive_structs` also run at warning level. Treat them as API and design review candidates; preserve a clearer or compatible design when that is the better trade-off.
