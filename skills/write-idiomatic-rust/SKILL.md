...
- The runner passes received options through to `cargo clippy`, then supplies its policy lints. For example, run `strict-clippy.sh --all-features` to validate all features. Inspect version-control status before using `--fix`; use it only when the relevant working tree is clean and its changes are committed. It passes standard output and standard error through unchanged. Then run the project's normal Clippy command separately.
